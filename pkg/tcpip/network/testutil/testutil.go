...
type MockLinkEndpoint struct {
	...
	mtu                uint32
	errorToInject      *tcpip.Error
	injectionCountdown int
}

// NewMockEndpoint create a new MockEndpoint.
//
// errorToInject can be set to an error that will be returned once
// injectionCountdown packets are written to the endpoint. If errorToInject is
// nil, no error will ever be returned.
func NewMockEndpoint(mtu uint32, errorToInject *tcpip.Error, injectionCountdown int) *MockLinkEndpoint {
	return &MockLinkEndpoint{
		mtu:                mtu,
		errorToInject:      errorToInject,
		injectionCountdown: injectionCountdown,
	}
}
...
func (ep *MockLinkEndpoint) WritePacket(_ *stack.Route, _ *stack.GSO, _ tcpip.NetworkProtocolNumber, pkt *stack.PacketBuffer) *tcpip.Error {
	if ep.errorToInject != nil {
		if ep.injectionCountdown == 0 {
			return ep.errorToInject
		}
		ep.injectionCountdown--
	}
...
func (ep *MockLinkEndpoint) WritePackets(_ *stack.Route, _ *stack.GSO, pkts stack.PacketBufferList, _ tcpip.NetworkProtocolNumber) (int, *tcpip.Error) {
	if ep.errorToInject != nil {
		if ep.injectionCountdown == 0 {
			return 0, ep.errorToInject
		}
	}

	writtenCount := ep.injectionCountdown
	if writtenCount > pkts.Len() {
		writtenCount = pkts.Len()
	}

	ep.injectionCountdown -= writtenCount
...
func (*MockLinkEndpoint) Attach(_ stack.NetworkDispatcher) {}
...
func (*MockLinkEndpoint) ARPHardwareType() header.ARPHardwareType { return header.ARPHardwareEther }
