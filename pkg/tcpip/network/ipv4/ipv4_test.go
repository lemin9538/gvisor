...
func TestFragmentation(t *testing.T) {
	...
	for _, ft := range fragTests {
		t.Run(ft.description, func(t *testing.T) {
			ep := testutil.NewMockEndpoint(ft.mtu, nil, 0)
			r := buildRoute(t, ep)
			...
			if got, want := len(ep.WrittenPackets), ft.expectedFrags; got != want {
...
func TestFragmentationErrors(t *testing.T) {
	...
	for _, ft := range fragTests {
		t.Run(ft.description, func(t *testing.T) {
			ep := testutil.NewMockEndpoint(ft.mtu, ft.injectError, ft.injectCountdown)
			r := buildRoute(t, ep)
			...
			if err != ft.injectError {
				t.Errorf("got WritePacket() = %v, want = %v", err, ft.injectError)
			}
...
func TestWriteStats(t *testing.T) {
	...
	for _, writer := range writers {
		t.Run(writer.name, func(t *testing.T) {
			for _, test := range tests {
				t.Run(test.name, func(t *testing.T) {
					ep := testutil.NewMockEndpoint(header.IPv4MinimumSize+header.UDPMinimumSize, tcpip.ErrInvalidEndpointState, test.expectWritten)
					rt := buildRoute(t, ep)
