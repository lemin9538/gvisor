...
type AddressableEndpoint interface {
	...
	// address.
	//
	// Returns tcpip.ErrBadLocalAddress if the receiver does not have the passed
	// permanent address.
...
func (a *AddressableEndpointState) RemoveAllPermanentAddresses() {
	a.mu.Lock()
	defer a.mu.Unlock()

	for _, ep := range a.mu.endpoints {
		// removePermanentEndpointLocked will return tcpip.ErrBadLocalAddress is ep
		// is not a permanent address.
		if err := a.removePermanentEndpointLocked(ep); err == tcpip.ErrBadLocalAddress {
			continue
		} else if err != nil {
