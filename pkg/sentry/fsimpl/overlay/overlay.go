...
func (fd *fileDescription) SetXattr(ctx context.Context, opts vfs.SetXattrOptions) error {
	...
	fs.renameMu.RLock()
	if err := fs.setXattrLocked(ctx, d, fd.vfsfd.Mount(), auth.CredentialsFromContext(ctx), &opts); err != nil {
		fs.renameMu.RUnlock()
		return err
	}
	fs.renameMu.RUnlock()
...
func (fd *fileDescription) RemoveXattr(ctx context.Context, name string) error {
	...
	fs.renameMu.RLock()
	if err := fs.removeXattrLocked(ctx, d, fd.vfsfd.Mount(), auth.CredentialsFromContext(ctx), name); err != nil {
		fs.renameMu.RUnlock()
		return err
	}
	fs.renameMu.RUnlock()
